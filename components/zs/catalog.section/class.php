...
class CatalogSectionComponent extends ElementList
{
	...
	protected function initCatalogInfo()
	{
        ...
        if ($this->useCatalog)
        {
            ...
            foreach ($this->globalFilter["IBLOCK_ID"] as $iblockId)
            {
        ...
        //echo "<pre>"; print_r($this->storage['CATALOGS']); echo "</pre>";
	...
	protected function getElementList($iblockId, $products) // elements
	{
    ...
    protected function processOffers()  // price offers
    {
        if ($this->useCatalog && !empty($this->globalFilter["IBLOCK_ID"]))
        {
            $offers = array();
            ...
            foreach ($this->globalFilter["IBLOCK_ID"] as $iblock)
            {
    ...
    protected function modifyDisplayProperties($iblock, &$iblockElements)
    {
        if (!empty($iblockElements)) {
            ...
            foreach ($iblockElements as $elementId => $arElement) {
                $iblock = $iblockElements[$elementId]["IBLOCK_ID"];
